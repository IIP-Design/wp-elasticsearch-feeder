...
function displayLevel( $terms, $selected, $parent = null ) {
  foreach ( $terms as $term ) :
    ...
    <option id="cdp-term-<?php echo $id; ?>" value="<?php echo $id; ?>" <?php echo ( in_array( $id, $selected ) ? 'selected="selected"' : '' ); ?>><?php echo ( $parent ? $parent->language->en . ' > ' : '' ); ?><?php echo $term->language->en; ?></option>
    <?php
    if ( count( $term->children ) ) {
      displayLevel( $term->children, $selected, $term );}
    ?>
    <?php
  endforeach;
}
